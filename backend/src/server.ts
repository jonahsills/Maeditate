...
// S3 client (for presigned PUT uploads)
...
// 4.2 Upload init - Return presigned PUT URL and public audio URL (Option C)
app.post('/v1/upload-init', authenticateToken, (async (req: Request, res: Response) => {
  try {
    ...
    // Object key layout: audio/<user>/<session>/<timestamp>-<rand>.<ext>
    ...
    // Public URL clients will pass back later
...
async function processAudioTranscription(transcriptId: string, audioUrlString: string, wantSummary: boolean) {
  try {
    ...
    // Decide source: remote (http) or local fallback
    let transcribedText = '';
    ...
    if (isRemote) {
      ...
      // Backward-compatible local storage path
...
// Helpers for remote audio processing
